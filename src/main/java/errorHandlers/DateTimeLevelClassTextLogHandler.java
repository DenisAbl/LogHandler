...
public class DateTimeLevelClassTextLogHandler extends BaseErrorHandler {

    private LogErrorHandler nextHandler;

    final private static String TIMESTAMP = "(?<timestamp>\\d{2}.\\d{2}.\\d{4}\\s\\d{2}:\\d{2}:\\d{2}.\\d{3})";
    final private static String LEVEL = "(?<level>ERROR)";
    final private static String CLASS = "(?<class>.*?)";
    final private static String TEXT = "(?<text>.*?)(?=\\d{2}.\\d{2}.\\d{4}\\s\\d{2}:\\d{2}:\\d{2}.\\d{3}|\\Z)";
    final private static String EXCEPTION = "(?<exception>\\S+\\.\\S+Exception)";
    private static Pattern patternLogFile = Pattern.compile(TIMESTAMP + " " + LEVEL + "\\s+" + CLASS + "\\s+-" + TEXT, Pattern.DOTALL);

    public DateTimeLevelClassTextLogHandler() {}
