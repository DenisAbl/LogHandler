...
public class TimeThreadLevelClassTextLogHandler extends BaseErrorHandler {

    private LogErrorHandler nextHandler;

    final private static String TIMESTAMP = "(?<timestamp>\\d{2}:\\d{2}:\\d{2})";
    ...
    final private static String TEXT = "(?<text>.*?)(?=\\d{2}:\\d{2}:\\d{2}\\s|\\Z)";
    ...
    public TimeThreadLevelClassTextLogHandler() {}
    ...
    public void handle(File file, Map<String, ErrorInfo> errorsMap){
                ...
                while (matcher.find()) {
                    ...
                    String hour = matcher.group("timestamp").substring(0,2);
